...
function index()
	entry({"admin", "services"}, alias("admin", "services", "phtunnel"), _("Services"), 50).index = true
	...
	local node = entry({"admin", "services", "phtunnel", "inner_status"}, template("oray/phtunnel_inner_status"), _("Inner Status"), 4)
	node.leaf = true
	node.hidden = true

	node = entry({"admin", "services", "phtunnel", "log_off"}, template("oray/phtunnel_log_off"), _("Log Off"), 5)
	node.leaf = true
	node.hidden = true
end
